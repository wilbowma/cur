...
(begin-for-syntax
 ...
 ; #:= (cur->datum #'(turn-λ (a : (turn-Type 0)) a)) '(λ (a : (Type 0)) a) ;lambda: bad syntax in (lambda (a) a)
  #:= (cur->datum #'(turn-Π (a : (turn-Type 0)) (turn-Type 0))) '(Π (a : (Type 0)) (Type 0))
  )
 (chk
  #:eq cur-equal? (cur-normalize #'(turn-Type 1)) #'(Type 1)
 ; #:eq cur-equal? (cur-normalize #'(turn-λ (x : (turn-Type 0)) x)) #'(λ (x : (Type 0)) x) ;fails: bad syntax in (lambda (x) x)
