...
(begin-for-syntax
  ...
  ; #:t (cur-type-check? #'Maybe  #'(cur-Π (A : (cur-Type 0)) (cur-Type 0))) ;fails: bad syntax in Maybe
 ...
 (chk
  #:eq (lambda (x y) (map cur-equal? x y)) (cur-constructors-for #'Nat) (list #'s #'z)
  #:eq (lambda (x y) (map cur-equal? x y)) (cur-constructors-for #'(Maybe Bool)) (list #'none #'just))
