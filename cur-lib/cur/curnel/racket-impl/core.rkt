  ...
  (define (reified-get-type e)
    (syntax-parse e
      ...
      [_
       ...
       (if x
           (syntax-local-introduce (let loop ([x x])
                                     (if (pair? x)
                                         (loop (car x))
                                         x)))
           (begin
...
(begin-for-syntax
  ...
  (define (cur-reflect syn)
    (syntax-parse syn
      ...
      [e:reified-pi
       (reified-copy-type (quasisyntax/loc syn (cur-Π (#,(cur-reflect #'e.name) : #,(cur-reflect #'e.ann))
                                   ;; TODO: subst should always be called on reified syntax?
                                   #,(subst (cur-reflect #'e.name) #'e.name (cur-reflect
                                                                             #'e.result))))
                          syn)]
      [e:reified-app
       (reified-copy-type (quasisyntax/loc syn (cur-app #,(cur-reflect #'e.rator) #,(cur-reflect
                                                                                     #'e.rand))) syn)]
      [e:reified-lambda
       (reified-copy-type (quasisyntax/loc syn (cur-λ (#,(cur-reflect #'e.name) : #,(cur-reflect #'e.ann))
